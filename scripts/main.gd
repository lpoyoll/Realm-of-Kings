...
func _update_nameplates() -> void:
	if camera_rig == null or not camera_rig.has_method("get_zoom_t"):
		return
	var zt: float = camera_rig.get_zoom_t()
	# Strategy band: fade all plates. Street: only ruler + named NPCs (or selected).
	var strategy_alpha: float = clampf(1.0 - (zt - 0.40) / 0.20, 0.0, 1.0)
	var in_strategy: bool = zt > 0.55
	for n in get_tree().get_nodes_in_group("nameplates"):
		if not (n is Label3D):
			continue
		var lab := n as Label3D
		var owner: Node = lab.get_parent()
		var is_important: bool = false
		if owner != null:
			is_important = owner.is_in_group("ruler") or owner.is_in_group("npcs")
			if selected != null and (owner == selected or lab.get_parent() == selected):
				is_important = true
		var alpha: float = 0.0
		var show: bool = false
		if in_strategy:
			alpha = strategy_alpha
			show = is_important and alpha > 0.05
		else:
			# Street: hide villager blob; keep important plates full
			show = is_important
			alpha = 1.0 if is_important else 0.0
		lab.visible = show
